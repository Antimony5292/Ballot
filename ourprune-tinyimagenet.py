...
if trial == 0:
    ...
    os.mkdir(root)
    model = timm.create_model('resnet50',num_classes=200,pretrained=False)
    model.to(device)
    ...
    if lth==True:
        ...
        for epoch in trange(0, EPOCHS):
            ...
            counter.append(count_conflict(model,'train_lth_{}.pkl'.format(prune_amount),train_loader,val_loader,root,num_classes=200))
            logger.info('Epoch:[{}/{}]\t loss={:.5f}\t acc={:.3f}'.format(epoch, EPOCHS, train_loss, acc))
            scheduler.step()
    ...
    if fairprune == True:
        ...
        custom_mask_fair = torch.zeros(200,2048)
        indx = torch.topk(cnt.view(-1), k)[1]
        custom_mask_fair.view(-1)[indx] += 1

        model = Load_model(root,'model_init.pkl',device=device,num_classes=200)
